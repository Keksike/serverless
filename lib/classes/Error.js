...
/**
 * Takes an Error and returns a object with important error- & environment-data.
 *
 * @param e {Error} - The error from which we build the object from.
 * @return {Object} - Built error- & env-data object.
 */
const getErrorObject = (e) => {
  let errorData = {
    type: e.name,
    message: e.message,
  };

  if (process.env.SLS_DEBUG) errorData.stack = e.stack;
...
module.exports.logError = (e, outputFormat) => {
  try {
    var eObj = getErrorObject(e);
    ...
    if (outputFormat === 'json') {
      ...
      if (process.env.SLS_DEBUG) {
        console.log(eObj);
        logStackTrace(eObj.error.stack);
      }
    ...
    throw new Error(errorHandlingError);
  }
};
